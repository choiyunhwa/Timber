...
//---------------
    static bool characChoose = true;

    // Prepare the player
    const int countPlayer = 3;
    const int characNum = 0;
    Texture texturePlayer[countPlayer];
    Sprite spritePlayer[countPlayer];
    texturePlayer[0].loadFromFile("graphics/player1.png");
    texturePlayer[1].loadFromFile("graphics/player2.png");
    texturePlayer[2].loadFromFile("graphics/player3.png");

    side playerSide[countPlayer];
    for (int i = 0; i < countPlayer; i++)
    {
        spritePlayer[i].setTexture(texturePlayer[i]);
        spritePlayer[i].setPosition(580, 720);
        playerSide[i] = side::LEFT;
    }

    spritePlayer[0].setPosition(600, 500);
    spritePlayer[1].setPosition(800, 500);
    spritePlayer[2].setPosition(1000, 500);
...
//----------------
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            case Event::KeyPressed:
                ...
                case Keyboard::Return:
                {
                    ...
                    spritePlayer[characNum].setPosition(580, 720);
                }
                break;
                case Keyboard::Left:
                    ...
                    {                        
                        chop.play();
                        {
                            playerSide[characNum] = side::LEFT;
                            ++score;                            
                ...
                case Keyboard::Right:
                    if (acceptInput && !isPause)
                    {
                        chop.play();
                        {
                            playerSide[characNum] = side::RIGHT;
                            ++score;                           
                            timeRemaining += (2.f / score) + 0.15f; //현재 스코어가 커질수록 채워지는 시간이 줄어듬
                            if (timeRemaining > timeMax)
                            {
                            ...
                            spritePlayer[characNum].setPosition(1200, 720);
                ...
                case Keyboard::Space:
                    if (characChoose)
                    {
                        ...
                        {
                        case 0:                            
                            spritePlayer[0].setPosition(580, 720);
                            break;
                        case 1:
                            spritePlayer[1].setPosition(580, 720);
                            break;
                        case 2:
                            spritePlayer[2].setPosition(580, 720);
                            break;
                        }
                        break;
                    }
        ...
        if (!isPause)
        {
            ...
            if (sideBranches[countBranches - 1] == playerSide[characNum]) //플레이어가 깔려서 죽을때
            {
                isPause = true;
                acceptInput = false;
                spritePlayer[characNum].setPosition(2000, 600);
        ...
        menu.draw(window);
        window.draw(spritePlayer[0]);
        window.draw(spritePlayer[1]);
        window.draw(spritePlayer[2]);// 플레이어
